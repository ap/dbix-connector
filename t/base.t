...
my $disco_meth = Hook::Guard->new( *DBI::db::disconnect )->hook( sub { ++$disconnect } );
...
# dbh inside a block.
BLOCK: {
    $ping_meth->hook( sub { pass 'Should not call ping()' } );
    is $conn->dbh, $dbh, 'Should get the database handle as usual';
    $ping_meth->hook( sub { fail 'Should not call ping() in a block' } );
...
APACHEDBI: {
    ...
    my $connect_meth = Hook::Guard->new( *DBI::connect )->hook( sub {
